...
// Sat Sep 24 14:36:58 EDT 2011
// This version has flash streaming and loading audio.  The two channels figure out when they are using the SD card and don't step on each others' toes.
// So, starting flash playback on any channel will stop flash playback on another channel and restart it.
// Also, all MIDI and button presses are locked when audio is saving or loading from the SD
// The next version of the firmware will try and play SD audio on its own interrupt to make this simpler if possible (and allow three channels of audio)
// Also, going for duophony on a given bank.
...
// New banked idea of the audio handler -- ONE FOR EACH BANK!
{
	...
	switch(bankStates[BANK_0].audioFunction)
	{
		...
		case AUDIO_PLAY_SD:		// Used to directly play audio streamed from the SD card.  Does not access ram at all, just passes the output byte back out.
		outputByte=adcByte;		// Pass the byte from the SD read fifo directly through.
		break;
...
//	PORTC|=Om_TEST_PIN;		// @@@ Used to time ISRs
	...
	if(bankStates[BANK_0].sdUsing)		// Handle SD/RAM transfers for this bank.  Not sample playback directly though.
	{
		if(bankStates[BANK_0].sdReading)	// Are we reading or writing this bank (the RAM) from the SD card?
		{
			...
			if(sdBytesInFifo<SD_FIFO_SIZE)	// Room in fifo?
			{
				...
				{
					sdFifo[sdFifoWritePointer]=UpdateAudioChannel0();	// Get next byte from RAM like you were playing back normally, and put it in the fifo.
					...
					midiOutputBank0=0;				// Don't pass any audio out on the DAC from this source.

					// Reset ISR and keep the ADC going
				...
				else	// No more bytes in the sample to be transferred.  Stop the ISR.
				{
					bankStates[BANK_0].audioFunction=AUDIO_IDLE;	// Stop audio ISR on this bank
					bankStates[BANK_0].clockMode=CLK_NONE;					
					bankStates[BANK_0].sdUsing=false;								// Mark this ISR as not sd related anymore.
				}			
			}			
		}
		else	// Writing the RAM (reading the SD)
		{
			if(sdRamSampleRemaining)	// Bytes remaining in the sample?
			{
				if(sdBytesInFifo)	// Anything currently in the FIFO?
				{
					adcByte=sdFifo[sdFifoReadPointer];		// Make the input byte to the record routine the byte we grab from the FIFO.
					midiOutputBank0=UpdateAudioChannel0();	// Put byte in RAM (using the record routine) or play it back (using sd playback routine) via the Audio handler
					UpdateOutput();							// Points to our current OP function.  Combines outputs from different sources and puts them on the DAC.  Shouldn't put anything on the DAC unless we're doing direct from SD playback.
			...
			else	// All sample bytes processed, end ISR
			{
				bankStates[BANK_0].audioFunction=AUDIO_IDLE;	// Stop audio ISR on this bank
				bankStates[BANK_0].clockMode=CLK_NONE;					
				bankStates[BANK_0].sdUsing=false;								// Mark this ISR as not sd related anymore.			
	...
	else	// Not loading/saving SD samples, just handle RAM based sampling
	{
		if((bankStates[BANK_0].halfSpeed==false)||(bankStates[BANK_0].halfSpeed&&flipFlop))		// Update the sample every ISR if we aren't at half speed, OR every other time if we are.
		{
...
//	PORTC|=Om_TEST_PIN;		// @@@ Used to time ISRs
	if(bankStates[BANK_1].sdUsing)		// Handle SD/RAM transfers for this bank.  Not sample playback directly though.
	{
		if(bankStates[BANK_1].sdReading)	// Are we reading or writing this bank (the RAM) from the SD card?
		{
			// Reading the RAM, writing the SD -- Get bytes from RAM, put them in fifo.  When fifo fills, pause.  When the entire sample has been transferred to the FIFO, stop the ISR
			if(sdBytesInFifo<SD_FIFO_SIZE)	// Room in fifo?
			{
				if(sdRamSampleRemaining)	// Any sample left in RAM?
				{
					sdFifo[sdFifoWritePointer]=UpdateAudioChannel1();	// Get next byte from RAM like you were playing back normally, and put it in the fifo.
					sdFifoWritePointer++;				// Move to next spot in fifo									
					...
					sdBytesInFifo++;				// One more byte in the FIFO
					sdRamSampleRemaining--;		// One less byte in the sample					
					midiOutputBank1=0;				// Don't pass any audio out on the DAC from this source.
				...
				{
					bankStates[BANK_1].audioFunction=AUDIO_IDLE;	// Stop audio ISR on this bank
					bankStates[BANK_1].clockMode=CLK_NONE;					
					bankStates[BANK_1].sdUsing=false;								// Mark this ISR as not sd related anymore.
				}			
			}			
		}
		else	// Writing the RAM (reading the SD)
		{
			if(sdRamSampleRemaining)	// Bytes remaining in the sample?
			{
				...
				{
					adcByte=sdFifo[sdFifoReadPointer];		// Make the input byte to the record routine the byte we grab from the FIFO.
					midiOutputBank1=UpdateAudioChannel1();	// Put byte in RAM (using the record routine) or play it back (using sd playback routine) via the Audio handler
					UpdateOutput();							// Points to our current OP function.  Combines outputs from different sources and puts them on the DAC.  Shouldn't put anything on the DAC unless we're doing direct from SD playback.
...
// Sat Apr 11 13:49:31 CDT 2009
{
	...
	bankStates[theBank].sdUsing=false;										// Bank not used for SD access
...
// NOTE: SD state machine will shut down the write process itself
{
	...
	if(cardState==SD_IDLE)	// SD card must be ready to do any of this
	{
		...
		bankStates[theBank].sdUsing=true;			// Bank is in use							
		bankStates[theBank].sdReading=true;			// Bank is being read (sd is being written)				
...
//		StartPlayback(theBank,CLK_INTERNAL,2048);				// Start "playing back" the sample into the SD card using internal clock at a high rate.  Well, not that high.  No need since we write so slow. 9.6k just for grins, though it does not need to be so fast.  That's 2048 in ticks.
		StartPlayback(theBank,CLK_INTERNAL,780);				// Start "playing back" the sample into the SD card using internal clock at a high rate.  Ratchet to 25kHz
//		StartPlayback(theBank,CLK_INTERNAL,600);				// Start "playing back" the sample into the SD card using internal clock at a high rate.  Ratchet to 32kHz
		bankStates[theBank].audioFunction=AUDIO_WRITE_SD;		// Mark this ISR as something besides playback (falls through to the playback function, but has a different name).
...
//		StartRecording(theBank,CLK_INTERNAL,2048);			//	Use the recording isr function to write stuff from the SD card to RAM via the fifo.  See above.
		StartRecording(theBank,CLK_INTERNAL,780);			//	Use the recording isr function to write stuff from the SD card to RAM via the fifo.  See above.
//		StartRecording(theBank,CLK_INTERNAL,600);			//	Use the recording isr function to write stuff from the SD card to RAM via the fifo.  See above.
		bankStates[theBank].audioFunction=AUDIO_READ_SD;	//  The RECORD isr is getting used, but we give it a separate marking so the program can differentiate

		// NOTE -- calling StartRecording will clear the sdUsing flag, so we have to set it true after the call.
		
		bankStates[theBank].sdUsing=true;			// Bank is in use							
		bankStates[theBank].sdReading=false;		// Bank is being written (sd is being read)			
...
// Reads a sample from the SD directly, and plays it without putting it in RAM first.  The sample is passed out through its own ISR.
{
	...
	{		
		...
		SetSampleClock(theBank,CLK_INTERNAL,907);			//  Start clock and set playback rate to 22050
		bankStates[theBank].audioFunction=AUDIO_PLAY_SD;	//  The audio handler has a special routine (just a passthrough) for handling this.

		SREG=sreg;	// resume isr
	}
	else if(bankStates[BANK_0].audioFunction==AUDIO_PLAY_SD||bankStates[BANK_1].audioFunction==AUDIO_PLAY_SD) // If we are already playing a sample from the SD, abort current SD stream, start new one
	// This requires finishing the current block read.  Be nice to not stop the ISRs and also not fuck up MIDI or encoders; this will require a change to the SD state machine or some other sort of creativity
	{
		...
		else	// Block is open for reading, so set up the SD state machine to end the read gracefully (and quickly) and then start the next one.
		{
			...
			bankStates[theBank].clockMode=CLK_NONE;	// Stop ISR clock
		}
	}
...
// Thu Aug  4 17:41:24 EDT 2011 -- Ought not to need these since we hit all those cases above
/*
...
// This state is based on the idea that playing, recording, and overdubbing are all discrete things and that if you're doing one you can't be doing another.
{
	...
	// Initialize everything.
	{
		...
		for(i=0;i<NUM_BANKS;i++)		// Initialize all the banks.
		{
			...
			bankStates[i].sdUsing=false;	// SD card not using anything yet
			bankStates[i].sdReading=false;	// SD card not reading anything yet
	...
	else if(subState==SS_1)		// Hang out here getting keypresses and MIDI and handling the different sampler functions.
	{
		if(editModeEntered==false)	// Normal functions for buttons?
		{
			...
			{
				if(bankStates[currentBank].audioFunction!=AUDIO_READ_SD&&bankStates[currentBank].audioFunction!=AUDIO_WRITE_SD) // check to see whether the bank is transferring data between SD and RAM and prevent switches from being recognized
				{
...
*/
			// NOTE -- we will drop any midi messages to a channel which is updating to/from SD

			if(bankStates[currentMidiMessage.channelNumber].audioFunction!=AUDIO_READ_SD&&bankStates[currentMidiMessage.channelNumber].audioFunction!=AUDIO_WRITE_SD) 		// check to see whether the bank is transferring data between SD and RAM and prevent MIDI from being recognized
			{
