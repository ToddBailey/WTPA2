...
static unsigned char UpdateAudio(void)
{
	static unsigned int
	...
	if (((rawData[currentAddress] >> dmcBitIndex) & 0x01) == 0)		// Right shift register driven by an 8-bit counter
	{
		deltaTrack -= 1;	
	}
	else 
...
static void OutputAudio(void)
{
	...
	sum0=(extIsrOutputBank0);		// Sum everything that might be involved in our output waveform:
...
{
	extIsrOutputBank0=UpdateAudio();		// If so, then call the audioIsr for bank 0 and do whatever it's currently supposed to do.
