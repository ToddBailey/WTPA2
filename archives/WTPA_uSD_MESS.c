...
// New banked idea of the audio handler -- ONE FOR EACH BANK!
{
	...
	switch(bankStates[BANK_0].audioFunction)
	{
		...
		if(sdReadingInto0==false)		// Normal write
		{
		...
		{
			if(sdBytesInFifo>0)	// Have stuff in the FIFO?
			{
				LATCH_PORT=sdFifo[sdFifoReadPointer];				// Put the data to write on the RAM's input port
...
*/
		...
		if(sdWritingFrom0)	// If we're sending data to the sd with this playback operation, route it to the fifo here.  Zero the actual output byte
		{
			sdFifo[sdFifoWritePointer]=outputByte;		// Put it in the fifo
			sdBytesInFifo++;							// Log it
			...
			if(sdBytesInFifo>=SD_FIFO_SIZE)		// Have we filled fifo?
			{
				// Need to pause isr.  We restart it in the sd state machine
				sdFifoPaused=true;
				bankStates[BANK_0].audioFunction=AUDIO_IDLE;
				bankStates[BANK_0].clockMode=CLK_NONE;				
			}
...
// New banked idea of the audio handler -- ONE FOR EACH BANK!
{
	...
	switch(bankStates[BANK_1].audioFunction)
	{
		...
		if(sdReadingInto1==false)		// Normal write
		{
			LATCH_PORT=adcByte;				// Put the data to write on the RAM's input port
		...
		if(sdWritingFrom1)	// If we're sending data to the sd with this playback operation, route it to the fifo here.  Zero the actual output byte
		{
			sdFifo[sdFifoWritePointer]=outputByte;		// Put it in the fifo
			sdBytesInFifo++;							// Log it
...
// Updates the state machine which keeps the card reads/writes/inits going like they should.
{
	...
	else	// Yup, got a card
	{
		switch(cardState)
		{
			...
			sdWritingFrom0=false;	// No weird ISR stuff	@@@ noided
			sdWritingFrom1=false;
			sdReadingInto0=false;	
			sdReadingInto1=false;	
...
//	zzz	ledOnOffMask=temp;		// Update the leds.
}
