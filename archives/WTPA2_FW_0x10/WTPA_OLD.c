...
// Olivier Gillet for the code review and some great ISR speed suggestions,
...
// External Flash Storage Stuff.
//-----------------------------------------------------------------------
static bool
	externalFlashEnabled;		// Set to true if we have a valid flash daughterboard attached

// RAM address, data, and DAC stuff.
//-----------------------------------------------------------------------

static volatile unsigned char
	isrType;					// Keeps track of what interrupt vector we're handling since the flag is cleared by hardware before the jump.  @@@ I think.

// Keys and analog reading variables.
//-----------------------------------------------------------------------
static unsigned char
	keyState,
	newKeys;
...
enum	// Flags we use to determine our IRQ source when servicing an interrupt.
{
	IRQ_CAPT=0,			// External Timer 1 capture event.
	IRQ_COMPA,			// Compare match A, Timer 1
	IRQ_COMPB,			// Compare match B, Timer 1
};

//==============================================
// Global Audio Output stuff:

static unsigned char
	audioOutputCombinationType;		// How do we put together the samples from the different banks before we put them on the DAC?
...
// Wed Mar 31 18:05:42 EDT 2010
// MADE THESE BANK SPECIFIC!
//-----------------------------------------------------------------------
//-----------------------------------------------------------------------
...
static unsigned char UpdateAudioChannel0(signed char adcInputByte)
// New banked idea of the audio handler -- ONE FOR EACH BANK!
{
	...
	switch(bankStates[BANK_0].audioFunction)
	{
		...
		outputByte=adcInputByte;				// Grab the value from the ADC, and put it back out.
		...
		LATCH_PORT=adcInputByte;				// Put the data to write on the RAM's input port
		...
		sum=outputByte+adcInputByte;			// Do saturated add mess.
		...
		adcInputByte=(signed char)sum;	// Back to 8 bits, please.

		LATCH_PORT=adcInputByte;	// Now replace the data at this RAM location with the data summed from the ADC and output bytes.
		PORTA&=~(Om_RAM_WE);		// Strobe Write Enable low.  This latches the data in.
		PORTA|=(Om_RAM_WE);			// Disbale writes.
...
static unsigned char UpdateAudioChannel1(signed char adcInputByte)
// New banked idea of the audio handler -- ONE FOR EACH BANK!
{
	signed int
		sum;			// For doing saturated adds.
...
static void DoInterruptHandler(void)
// Calls the audio interrupt as needed for the different banks and clock sources.
// Vector here from directly from IRQa and then figure out what you need to do.
// We could make the bank system a "1 or (n)" case handling loop here, but I've unrolled it to go faster in the ISR.  Adding more banks will complicate this.
// NOTE:  Can't use an ALIASOF in the different ISRs anymore because we now need to know what got us to the interrupt handler.  We can't test the Interrupt Flags because they'll be cleared before we
// vector here.  So we just set our own flag in either interrupt vector and then jump here.
// Right now we have 3 possible clock sources.  This is a little less general than I'd like, but I did it so that we'd be able to separate the pitch of the two independent MIDI outputs.
// The external clock source (an edge trigger on TIMER1 input compare) can generate audio on either bank.
// The internal clock is based on TIMER1's output compares, and can generate two different interrupts -- OCR1A and OCR1B.  These are explicitly associated with BANK_0 and BANK_1 respectively.
// I could not see a way to make the pitch of the two midi samples independent without doing this.  As it stands, the pitch of the two banks is not independent when using the external interrupts,
// but I decided the extra complexity was worth it to separate the pitches for the two midi channels.

// On signage:  The adc data comes in unsigned and we make it a signed twos complement char.  It stays this way until we spit it back out again on the DAC because this makes summing the independent audio channels easier.
// On the audio channels:  We need to know the difference between an interrupt source which is ACTIVE but not being triggered in this current call now and one which is not active.
//		One which is active should retain its value even if it isn't being updated, and one which is inactive should default to midscale.  The idle channels have their contributions negated in the switch statement before the sum.

// On Jitter:
// It's awful slow the way I've done it.
{
	static signed char
	...
	static bool
		flipFlop;			// Used to make us run in halftime.
...
//		multiplyTrickery;
	...
	static unsigned char
		lastDacByte;	// Very possible we haven't changed output values since last time (like for instance we're recording) so don't bother strobing it out.
	unsigned char
		dacByte;		// What will we eventually put on the DAC?

// Sun Sep 19 13:16:10 EDT 2010
// TMB -- retroactive
// Starting the ADC read here TOTALLY puts it in line with the pins flopping on PORTA during RAM access, and is MANY times noisier than at the end of the handler.  See the end of the handler.
	...
	switch(isrType)
	{
		case IRQ_CAPT:		// An input compare got us here -- the edge from the external clock.  Either or both banks can trigger on the external clock.
		if(bankStates[BANK_0].clockMode==CLK_EXTERNAL)		// Is bank 0 supposed to be triggering off the external clock?
		{
			...
			{
				extIsrOutputBank0=UpdateAudioChannel0(adcByte);		// If so, then call the audioIsr for bank 0 and do whatever it's currently supposed to do.
			}
		}
		if(bankStates[BANK_1].clockMode==CLK_EXTERNAL)		// Is bank 1 supposed to be triggering off the external clock?
		{
			...
			{
				extIsrOutputBank1=UpdateAudioChannel1(adcByte);		// If so, then call the audioIsr for bank 1 and do whatever it's currently supposed to do.
			}
		}
		break;
		
		case IRQ_COMPA:		// Did we get here because of an interrupt triggered from the internally generated TIMER1 OCR1A clock (BANK_0 and MIDI related)?
		if(bankStates[BANK_0].clockMode==CLK_INTERNAL)		// Is bank 0 supposed to be triggering off the internal (midi) clock?
		{
			if((bankStates[BANK_0].halfSpeed==false)||(bankStates[BANK_0].halfSpeed&&flipFlop))		// Update the sample every ISR if we aren't at half speed, OR every other time if we are.
			{
				midiOutputBank0=UpdateAudioChannel0(adcByte);			// If so, then call the audioIsr for bank 0 and do whatever it's currently supposed to do.
			}
			if(bankStates[BANK_0].jitterValue)				// Jitter on?		### This math is wrong, or, more likely, this routine is too slow.  Once the jitterValue gets reasonably high we here the samples slow down...
			{
				...
				OCR1A+=(bankStates[BANK_0].timerCyclesForNextNote-jitterTemp)+lastJitterValue0;		// To our OCR value we now add the normal time until the next interrupt MINUS some random number.  This gives us the jitter.  Then we add in the leftovers from the last jitter event, and this keeps our period constant. ### We can easily roll this register around for slow notes, but fuck it, this is about jitter, right?
				lastJitterValue0=(unsigned int)jitterTemp;											// Store our jitter as an offset for next time; this keeps our period constant.
			}
			else
			{
		...
		case IRQ_COMPB:		// Got here b/c of internally generated TIMER1 OCR1B clock (BANK_1 and MIDI related).
		if(bankStates[BANK_1].clockMode==CLK_INTERNAL)		// Is bank 1 supposed to be triggering off the internal (midi) clock?
		{
			if((bankStates[BANK_1].halfSpeed==false)||(bankStates[BANK_1].halfSpeed&&flipFlop))		// Update the sample every ISR if we aren't at half speed, OR every other time if we are.
			{
				midiOutputBank1=UpdateAudioChannel1(adcByte);		// If so, then call the audioIsr for bank 1 and do whatever it's currently supposed to do.
			}
			if(bankStates[BANK_1].jitterValue)				// Jitter on?
			{
				...
				jitterTemp=random31%(jitterTemp/JITTER_VALUE_MAX);									// Pick a random value between max and min possible jitter (when jitterValue == JITTER_VALUE_MAX this will be a random number from 0 to timerCyclesForNextNote).
				OCR1B+=(bankStates[BANK_1].timerCyclesForNextNote-jitterTemp)+lastJitterValue1;		// To our OCR value we now add the normal time until the next interrupt MINUS some random number.  This gives us the jitter.  Then we add in the leftovers from the last jitter event, and this keeps our period constant. ### We can easily roll this register around for slow notes, but fuck it, this is about jitter, right?
				lastJitterValue1=(unsigned int)jitterTemp;											// Store our jitter as an offset for next time; this keeps our period constant.
			}
			else
			{
	...
	if(flipFlop)			// Toggle our half-speed flip flop.   @@@ can we do this with an XOR?  like with a bit toggle?  What's faster?
	{
		flipFlop=false;
	}
	else
...
// Now void any input to the DAC from interrupts that aren't happening:	

	switch(bankStates[BANK_0].clockMode)
	{
		case CLK_NONE:				// No clock on this channel means no output from either source.
		extIsrOutputBank0=0;
		midiOutputBank0=0;
		break;		
	
		case CLK_EXTERNAL:			// External clock on the bank means no midi output for this channel / bank.
		midiOutputBank0=0;
		break;
		
		case CLK_INTERNAL:			// Internal clock on the bank means no caveman output for this channel / bank.
		extIsrOutputBank0=0;
		break;
	...
	switch(bankStates[BANK_1].clockMode)
	{
		case CLK_NONE:				// No clock on this channel means no output from either source.
		extIsrOutputBank1=0;
		midiOutputBank1=0;
...
// Now decide how to put the audio channels together before outputting them.  A sum is the normal way to do this, but we're on that "next level" shit.
// We repeat a lot of code here because we have more flash memory than time in the audio interrupt.

	switch(audioOutputCombinationType)
	{
		case OUTPUT_MULTIPLY:
...
//		multiplyTrickery=(sum0*sum1)^0x8000;
//		if(multiplyTrickery>0x3FC0)
//		{
//			multiplyTrickery=0x3FC0;	// 255 shifted left by 6.
//		}
//		dacByte=(multiplyTrickery>>6);

//		multiplyTrickery=(sum0*sum1)^0x8000;
//		dacByte=(multiplyTrickery/128);

//		The following one works without vecoring to the ugly divison routine, but is quiet.
//		multiplyTrickery=(sum0*sum1)^0x8000;
//		dacByte=(multiplyTrickery>>8);

//		sum0=(sum0*sum1);
//		dacByte=(unsigned char)(((sum0>>8)&0xFF)^0x80);	// Unsign.
...
// Removed need to pin to range by limiting excursion above
...
//		sum0=((sum0*sum1)/127);				// Multiply the sums of the banks, and divide them down to full scale output.  If this sounds too tame, we may want to make the divisor smaller and pin this to range as above.
//		sum0=((signed char)sum0)*((signed char)sum1);
//		sum0/=127;
		dacByte=(((signed char)sum0)^0x80);	// Cast the output back to 8 bits and then make it unsigned.
		break;
		...
		dacByte=(((signed char)sum0)^0x80)^(((signed char)sum1)^0x80);		// Cast each sum back to 8 bits, make them unsigned, then xor them.		
		break;
		...
		dacByte=(((signed char)sum0)^0x80)&(((signed char)sum1)^0x80);		// Cast each sum back to 8 bits, make them unsigned, then and them.		
		break;
		...
		dacByte=(signed char)sum0;		// Cast back to 8 bits.
		dacByte^=(0x80);				// Make unsigned again (shift 0 up to 128, -127 up to 0, etc)
		break;
		
		default:
		dacByte=128;		// No diggity.
		break;	
	}
	
// Now actually spit the byte out if it's different than the last thing we put out.

	if(dacByte!=lastDacByte)	// Is this byte already on the latch?
	{
		PORTA|=(Om_RAM_OE);		// Tristate the RAM.
		LATCH_DDR=0xFF;			// Turn the data bus around (AVR's data port to outputs)

		LATCH_PORT=dacByte;		// Put the dacByte on the output latch's input.
...
// Sun Sep 19 13:15:34 EDT 2010
// TMB, retroactive
//  Putting the beginning of the ADC read here TOTALLY FIXES THE DIGITAL NOISE!
//	if(!(ADCSRA&(1<<ADSC)))			// Last conversion done (note that once we start using different clock sources it's really possible to read this too often, so always check to make sure a conversion is done)
//	{
//		adcByte=(ADCH^0x80);		// Update our ADC conversion variable.  If we're really flying or using both interrupt sources we may use this value more than once.  Make it a signed char.
//		ADCSRA |= (1<<ADSC);  		// Start the next conversion.
...
{
	PORTC|=Om_TEST_PIN;		// @@@ Used to time ISRs
	isrType=IRQ_CAPT;		// Set a flag so we know which vector sent us to the interrupt handler.
	DoInterruptHandler();	// Do all the audio work.
}

ISR(TIMER1_COMPA_vect)
{
	PORTC|=Om_TEST_PIN;		// @@@ Used to time ISRs
	isrType=IRQ_COMPA;		// Set a flag so we know which vector sent us to the interrupt handler.
	DoInterruptHandler();	// Do all the audio work.
}
...
{
	PORTC|=Om_TEST_PIN;		// @@@ Used to time ISRs
	isrType=IRQ_COMPB;		// Set a flag so we know which vector sent us to the interrupt handler.
	DoInterruptHandler();	// Do all the audio work.
}
...
{
	if(bankStates[theBank].clockMode==CLK_INTERNAL)			// The internally counted clock is usually associated with MIDI-controlled sampling (output capture on timer 1)
	{
	...
	else if(bankStates[theBank].clockMode==CLK_EXTERNAL)	// External clock is based on Input Capture interrupts (rising edge from the sample clock oscillator)
	{
...
// Sat Apr 11 13:49:31 CDT 2009
{
	...
	bankStates[theBank].clockMode=theClock;									// What type of interrupt should trigger this sample bank?
...
{
	if((bankStates[BANK_0].clockMode!=CLK_EXTERNAL)&&(bankStates[BANK_1].clockMode!=CLK_EXTERNAL))	// If neither bank is using the external interrupt...
	{
...
// This state is based on the idea that playing, recording, and overdubbing are all discrete things and that if you're doing one you can't be doing another.
{
	...
	// Initialize everything.
	{
		...
		audioOutputCombinationType=OUTPUT_SUM;	// Don't do any crazy shit until the user says.
		...
		audioOutputCombinationType=OUTPUT_SUM;
				...
				else
				{
					...
					if(newKeys&Im_SWITCH_2)		// Switch 2 assigns our different ways of combining audio channels on the output.
					{
						switch(scaledEncoderValue)
						{
							...
							case 1:
							audioOutputCombinationType=OUTPUT_MULTIPLY;
							PutMidiMessageInOutgoingFifo(currentBank,MESSAGE_TYPE_CONTROL_CHANGE,MIDI_OUTPUT_COMBINATION,scaledEncoderValue);		// Send it out to the techno nerds.
							break;
							
							case 2:
							audioOutputCombinationType=OUTPUT_AND;
							PutMidiMessageInOutgoingFifo(currentBank,MESSAGE_TYPE_CONTROL_CHANGE,MIDI_OUTPUT_COMBINATION,scaledEncoderValue);		// Send it out to the techno nerds.
							break;
							
							case 3:
							audioOutputCombinationType=OUTPUT_XOR;
							PutMidiMessageInOutgoingFifo(currentBank,MESSAGE_TYPE_CONTROL_CHANGE,MIDI_OUTPUT_COMBINATION,scaledEncoderValue);		// Send it out to the techno nerds.
							break;
...
// Initialize this mess.
{
	...
	DDRC=0xFF;			// PORTC is the switch latch OE and direct address line outputs which must be initialized here.  Pins PC4-PC7 are unused now, so pull them low.
