...
static void PlaySample(char sampleNumber)
{
	for(int i=0; i < 4; i++)
	{
		if(dmcChannels[i].isPlaying == 0)
		{
			...
			dmcChannels[i].currentAddress = sampleNumber * 4096; //*4096	// Which 4K sample? (could << 12 here)
			dmcChannels[i].endAddress = (sampleNumber+1) * 4096; //*4096	// Use the whole bank for now - until I figured out a formate for the length
...
static signed char UpdateChannel(unsigned char i)
{
	...
	if(dmcChannels[i].isPlaying == 1)
	{
		...
		if(dmcChannels[i].currentAddress == dmcChannels[i].endAddress)	// Hardcoded to sample length - reset when we've reached the end of the sample
		{
